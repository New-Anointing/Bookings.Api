...
namespace Bookings.Modules.Ticketing.Application.Carts;
...
public sealed class CartService(ICacheService cacheService)
{
    ...
    public async Task<Cart> GetAsync(Guid customerId, CancellationToken cancellationToken)
    {
        string cashKey = CreateCacheKey(customerId);

        Cart cart = await cacheService.GetAsync<Cart>(cashKey, cancellationToken) ??
    ...
    public async Task AddItemAsync(Guid customerId, CartItem cartItem, CancellationToken cancellationToken)
    {
        string cacheKey = CreateCacheKey(customerId);
